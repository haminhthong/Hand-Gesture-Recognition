"""Mô-đun máy trạng thái hữu hạn (Dynamic FSM) nhận diện chuỗi cử chỉ động On/Off."""
...
class DynamicGestureFSM:
    """Máy trạng thái hữu hạn nhận diện chuỗi cử chỉ động On/Off.

    Quy trình FSM của On/Off:
    - Trạng thái bắt đầu (START_ONOFF): Ngón trỏ và ngón giữa duỗi, ngón giữa chụm vào ngón cái.
    - Trạng thái hoàn tất: Trong khoảng thời gian timeout, ngón giữa mở ra (không còn chạm ngón cái).
      Phát sinh sự kiện "On/Off" một lần duy nhất để bật/tắt canvas tương tác.
    """

    def __init__(self, timeout_seconds: float = 1.5) -> None:
        if timeout_seconds <= 0:
            raise ValueError("Timeout của dynamic FSM phải lớn hơn 0.")
    ...
    def _is_finger_extended(
        ...
        """Kiểm tra ngón tay duỗi dựa trên khoảng cách tới cổ tay."""
        wrist = landmarks[0]
    ...
    def update(self, observation: Optional[HandObservation]) -> str:
        ...
        Returns:
            str: "On/Off" nếu hoàn tất chuỗi cử chỉ, ngược lại trả về "Still".
        """
        ...
        if self.current_state != "IDLE":
            if now - self.state_start_time > self.timeout_seconds:
                self.reset()

        # FSM Transition Logic
        if self._is_on_off_start(landmarks, palm_size, finger_states):
            if self.current_state != "START_ONOFF":
                self.current_state = "START_ONOFF"
